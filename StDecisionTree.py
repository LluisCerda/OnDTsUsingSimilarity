...
class StDecisionTree:
    ...
    def _build_tree(self, X, y, depth):
        ...
        # Classification: Stop if pure leaf node
        if self.task == "classification" and len(unique_labels) == 1:
            return unique_labels[0]  # Pure class
        
        # Regression: Stop if depth limit reached, return mean value
        if self.task == "regression" and (self.max_depth is not None and depth >= self.max_depth):
            return np.mean(y)

        # Classification: Stop if max depth reached, return majority class
        if self.task == "classification" and (self.max_depth is not None and depth >= self.max_depth):
    ...
    def _best_split(self, X, y):
        ...
        for feature in range(num_features):
            ...
            for threshold in thresholds:
                ...
                score = self._gini_index(y[left_indices], y[right_indices]) if self.task == "classification" else self._mse(y[left_indices], y[right_indices])
    ...
    def visualize_tree_pyvis(self, output_file="tree.html", feature_names=None, class_names=None):
