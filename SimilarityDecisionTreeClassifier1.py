...
class SimilarityDecisionTreeClassifier1:
    ...
    def _compute_medoid(self, X):
        ...
        categorical_indices=utils.get_categorical_indices(isCategorical=self.isCategorical)
        
        if len(categorical_indices) > 0: distances = self.gower_distance(X, cat_features=categorical_indices)  
        else: distances = self.gower_distance(X)  
    ...
    def _traverse_tree(self, x, node):
        ...
        if len(categorical_indices) > 0: distance = self.gower_distance(x.reshape(1, -1), node["medoid"].reshape(1, -1), cat_features=categorical_indices)  
        else: distance = self.gower_distance(x.reshape(1, -1), node["medoid"].reshape(1, -1))  
